 ...
 *   GHL_API_TOKEN            GoHighLevel Private Integration token (preferred mode:
 *                            direct contact upsert + workflow enrollment).
 *   GHL_LOCATION_ID          GoHighLevel location (sub-account) ID.
 *   GHL_FDD_WORKFLOW_ID      Workflow the contact is enrolled in directly.
 *   GHL_FDD_WEBHOOK_URL      Inbound-webhook trigger (used when no API token is set).
...
/**
 * Non-secret defaults for the single-brand deployment (the API token has no
 * default — it stays env-only). Location: Complete Mobile Drug Testing;
 * workflow: "02 - Move to Stage 3 send FDD". Env vars override both.
 */
const DEFAULT_GHL_LOCATION_ID = "4dwsLmHGWb6ElyIQlZOc";
const DEFAULT_GHL_FDD_WORKFLOW_ID = "560a0e6d-057f-490f-8c8d-2df19f06272b";
...
export function getGhlConfig(): GhlConfig {
  return {
    ...
    locationId: process.env.GHL_LOCATION_ID ?? DEFAULT_GHL_LOCATION_ID,
    workflowId: process.env.GHL_FDD_WORKFLOW_ID ?? DEFAULT_GHL_FDD_WORKFLOW_ID,
