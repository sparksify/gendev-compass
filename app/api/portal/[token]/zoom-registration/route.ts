...
export async function POST(
  ...
  if (!parsed.success) {
    return NextResponse.json({ success: false, error: "Choose a valid session." }, { status: 400 });
  }
  try {
    ...
    return NextResponse.json(
      { success: false, error: "We couldn’t complete your Zoom registration. Please try again." },
      { status: 502 },
    );
  }
}
